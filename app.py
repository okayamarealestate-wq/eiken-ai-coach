...
def summarize_progress(df: pd.DataFrame, days: int = 7) -> Dict[str, float]:
    ...
    return {
        ...
        "understanding": float(recent["understanding"].fillna(0).mean() if len(recent) else 0),
    }
...
def coach_comment(settings: Dict, plan: Dict, df: pd.DataFrame) -> str:
    ...
    stage_line = "2級では文法と型を安定させるのが大事。" if stage == "英検2級" else "準1級では語彙・要約・長文への耐性を伸ばすのが大事。"
...
def coach_bubble_html(settings: Dict, message: str) -> str:
    ...
    return (
        ...
        f"<div style='width:54px; height:54px; border-radius:50%; background:#f3f4f6; display:flex; align-items:center; justify-content:center; font-size:26px; border:2px solid {border_color};'>🧑‍🏫</div>"
        f"<div style='position:relative; background:{bubble_color}; border:2px solid {border_color}; border-radius:18px; padding:14px 16px; max-width:100%; box-shadow:0 2px 6px rgba(0,0,0,0.05);'>"
        f"<div style='font-weight:700; margin-bottom:6px;'>{name} コーチ</div>"
...
st.title(APP_TITLE)
st.caption("Google Sheets保存、2級→準1級の自動切替、ゲーム要素、キャラクターコーチ、吹き出しUIに対応した家庭用英検学習ダッシュボード")
...
st.divider()
st.caption("使い方: ①Google Sheets を設定すると公開版でもログが残る → ②コーチを設定 → ③2級モードで学習 → ④合格なら準1級モードへ自動切替。")
